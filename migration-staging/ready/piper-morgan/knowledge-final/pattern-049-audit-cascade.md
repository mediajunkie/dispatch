...
LLMs, like their human trainers, struggle to adhere to checklists during creation. However, they excel at auditing work against checklists and making corrections. This asymmetryâ€”poor at following templates while creating, excellent at verifying against templates after creatingâ€”is a fundamental characteristic that this pattern exploits.
...
Multi-step work (issues â†’ gameplans â†’ prompts â†’ execution) accumulates drift at each handoff:
- Issue written without consulting template â†’ gaps
- Gameplan based on incomplete issue â†’ inherited gaps + new gaps
- Prompts based on flawed gameplan â†’ compounded errors
- Execution based on flawed prompts â†’ failure
...
â”‚  Write  â”‚â”€â”€â”€â”€â–¶â”‚  Audit  â”‚â”€â”€â”€â”€â–¶â”‚  Write   â”‚â”€â”€â”€â”€â–¶â”‚  Audit  â”‚â”€â”€â”€â”€â–¶â”‚  Write  â”‚â”€â”€â”€â”€â–¶â”‚  Audit  â”‚â”€â”€â”€â”€â–¶ Execute
â”‚  Issue  â”‚     â”‚  Issue  â”‚     â”‚ Gameplan â”‚     â”‚Gameplan â”‚     â”‚ Prompts â”‚     â”‚ Prompts â”‚
...
2. **Audit issue** against template â†’ fix gaps (there are always gaps, even when template was consulted)
3. **Write gameplan** based on audited issue; discuss any open questions
4. **Audit gameplan** against template â†’ fix gaps
5. **Write prompts** for subagents based on audited gameplan
6. **Audit prompts** against template â†’ fix gaps
...
2. Create a checklist matrix (template section â†’ current document)
3. Mark each requirement as âœ… present, âš ï¸ partial, or âŒ missing
4. Fix all âš ï¸ and âŒ items before proceeding
...
|---------------------|--------|-------|
| Issue number referenced | âœ… | #583 |
| Problem statement | âœ… | Chat not persisting on refresh |
| Five-whys analysis | âš ï¸ | Only 3 whys - need deeper |
| Success criteria | âœ… | 3 criteria defined |
| Test strategy | âŒ | Missing - add TDD approach |
| Phases with estimates | âœ… | 4 phases, 2-3 hours |
| Rollback plan | âŒ | Missing - add |
...
â†’ This is exactly when drift occurs. The audit catches assumptions you don't know you're making.

### Auditing Without Templates
"I'll just review this carefully."
â†’ Without a checklist, you'll miss the same things you missed when writing. The template externalizes the requirements.

### Multiple Redundant Audits
"I'll audit three times to be sure."
â†’ One thorough audit against template is sufficient. Multiple passes have diminishing returns.
...
The word "audit" mattersâ€”it triggers systematic, thorough verification behavior.
